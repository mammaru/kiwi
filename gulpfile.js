...
var serveDir    = 'comp';   // directory for serve task
...
gulp.task('compile:styles', function () {
    return gulp.src([srcDir + '/static/**/*.scss'])
        .pipe($.sourcemaps.init())
        .pipe($.sass())
        .pipe($.sourcemaps.write('.'))
        .pipe(gulp.dest(serveDir + '/static'))
    ;
});
...
gulp.task('inject:css', ['compile:styles'], function() {
    return gulp.src(srcDir + '/**/*.html')
        .pipe($.inject(gulp.src(mainBowerFiles().concat([serveDir + '/static/**/*.css']), {read: false}), {
            relative: true,
            //name: 'bower'
            ignorePath: ['../comp/', '..'],
            addPrefix: '..'
...
gulp.task('html', ['inject:css'], function () {
    var assets = $.useref.assets({searchPath: ['bower_components', serveDir + '/static']});
...
gulp.task('copy:fonts', function () {
    return gulp.src('bower_components/**/fonts/*.woff*')
        .pipe($.flatten())
        .pipe(gulp.dest(serveDir + '/static/fonts'))
...
gulp.task('misc', ['copy:static', 'copy:fonts']);

gulp.task('serve', ['inject:css', 'compile:scripts:watch', 'compile:styles', 'misc'], function () {
    var electron = electronServer.create();
    electron.start();
    gulp.watch(['bower.json', srcDir + '/index.html'], ['inject:css']);
    gulp.watch([serveDir + '/app.js', serveDir + '/browser/**/*.js'], electron.restart);
    gulp.watch([serveDir + '/static/**/*.css', serveDir + '/**/*.html', serveDir + '/renderer/**/*.js'], electron.reload);
});

gulp.task('build', ['html', 'compile:scripts', 'packageJson', 'misc']);
